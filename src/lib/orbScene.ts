...
export type OrbVisualState = "idle" | "thinking" | "responding" | "listening";
...
export interface OrbSceneApi {
  ...
  zoomOut(): void;
  resetView(): void;
  /** Update the orb's visual state (changes bloom, pulse, scan speed). */
  setOrbState(state: OrbVisualState): void;
  dispose(): void;
}
...
/* ── Orb state → visual parameters ── */
interface OrbVisualParams {
  bloomStrength: number;
  bloomPulse: number;   // amplitude of bloom oscillation
  bloomSpeed: number;   // speed of bloom oscillation
  scanSpeed: number;     // multiplier on scan ring sweep speed
  corePulse: number;     // multiplier on core surge intensity
}

const STATE_PARAMS: Record<OrbVisualState, OrbVisualParams> = {
  idle:       { bloomStrength: 1.6, bloomPulse: 0.3, bloomSpeed: 0.8, scanSpeed: 1.0, corePulse: 1.0 },
  listening:  { bloomStrength: 2.0, bloomPulse: 0.5, bloomSpeed: 1.2, scanSpeed: 1.6, corePulse: 1.4 },
  thinking:   { bloomStrength: 1.8, bloomPulse: 0.6, bloomSpeed: 1.5, scanSpeed: 1.3, corePulse: 1.2 },
  responding: { bloomStrength: 2.4, bloomPulse: 0.4, bloomSpeed: 1.0, scanSpeed: 2.0, corePulse: 1.8 },
};
...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  // ——— ORB STATE ———
  let currentOrbState: OrbVisualState = "idle";
  ...
  /**
   * Build a single texture atlas with N tinted copies of each snippet, so
   * thousands of text sprites can share one texture instead of each having
   * its own canvas + GPU upload. This is what makes the orb usable on the
   * main thread — 1,700 individual canvases froze the page for ~50s.
   *
   * Returns: { texture, sprites } where each sprite references a row in the
   * atlas and can be tinted/positioned independently.
   */
  function buildTextAtlas(count: number): { texture: THREE.CanvasTexture; tiles: { x: number; w: number }[] } {
    // Lay each snippet out as a horizontal strip of `count` tinted copies.
    // One row per snippet, one column per tint.
    const unique = codeSnippets.length;
    const cellW = 64;
    const cellH = 16;
    ...
    for (let s = 0; s < unique; s++) {
      for (let c = 0; c < count; c++) {
        ...
        const cy = s * cellH + cellH / 2;
        ctx.fillText(codeSnippets[s], cx, cy);
        tiles.push({ x: c * cellW, w: cellW });
      }
    }
  ...
  // On outer sphere — text coverage (kept lean to avoid main-thread freezes)
  ...
  function animate() {
    ...
    const params = STATE_PARAMS[currentOrbState];
    ...
    // Core pulse — dramatic surges, intensity scaled by orb state
    const coreMul = params.corePulse;
    ...
    const surge = (wave3 * 1.5 + wave4 * 2.0) * coreMul;
    ...
    // Scan rings sweeping — speed reacts to orb state
    const scanSpeed = params.scanSpeed;
    const scanY1 = Math.sin(t * 0.4 * scanSpeed) * R1;
    ...
    const scanY2 = Math.sin((t * 0.6 + 2) * scanSpeed) * R3;
    ...
    // Bloom pulse — reacts to orb state
    bloom.strength = params.bloomStrength + Math.sin(t * params.bloomSpeed) * params.bloomPulse;
  ...
  function setOrbState(state: OrbVisualState) {
